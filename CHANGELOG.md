...
- Shipped **Hold** ($12): Notion CSVs + Excel workbook for waiting-on-client blockers (Projects, Blockers, Nudges, Unblocks, Week_Review).
- Shipped **Helm** ($12): Notion CSVs + Excel workbook for discovery-call intake (Calls, Fit_Score, Next_Steps, Notes, Week_Review).
...
- `index.html` — footer badge (required for free submit)
- `overdue-invoice-emails.html` — same badge
- `freelance-invoice-spreadsheet.html` — same badge

No GitHub push from this change (browser push by parent).

## 2026-09-10 — Product upgrade mentions (no new SKUs)

Funnel copy updated to note in-place upgrades on existing Gumroad products:

- **Ops** (`ggnfxs`, still $12): ZIP now includes overdue lite HTML with CSV import + Print.
- **Slip** (`tubrnw`, free): HTML print polish + due date / pay-by note fields.
- **Nudge** (`ebexb`, free): PDF now 8 pages with a subject-line swipe file.

### Pages touched

- `index.html` — Nudge / Slip / Ops cards + “How it fits together” steps
- `overdue-invoice-emails.html` — intro, after-they-pay Slip note, soft CTA
- `freelance-invoice-spreadsheet.html` — Ops ZIP note, soft CTA, Nudge/Slip refs

Navy/red branding, Gumroad CTAs, LaunchFree and Spotlitely links unchanged.
